...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ success: true, message: '已確認送達，目標據點庫存已更新', log: data }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
  } catch (e) {
    return new Response(JSON.stringify({ success: false, message: e instanceof Error ? e.message : '確認失敗' }), { status: e instanceof AppError ? e.status : 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
  }
})
