...
serve(async(req)=>{if(req.method==='OPTIONS')return new Response('ok',{headers:cors});try{
 ...
 if(body.action==='create'){
  ...
  const email=`${body.username.trim()}@local.invalid`
  const {data:created,error}=await admin.auth.admin.createUser({email,password:body.password,email_confirm:true}); if(error)throw new E(error.message)
  const {error:pErr}=await admin.from('profiles').insert({id:created.user.id,username:body.username.trim(),display_name:body.displayName?.trim()||null,role_name:body.roleName,location_id:body.locationId||null,is_active:body.isActive!==false})
  if(pErr){await admin.auth.admin.deleteUser(created.user.id);throw new E(pErr.message)}
