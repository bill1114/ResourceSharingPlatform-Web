...
| 分類 | 功能 |
|---|---|
| 戰情總覽 | 據點/全系統低庫存、即期/過期統計、各據點物資統計 |
| 物資管理 | 物資清單（篩選、跨據點統計、詳細/編輯/轉移/刪除、Excel 匯出）、物資入庫 |
| 物資異動 | 出庫、捐贈、報廢、跨據點轉移（建立→確認→取消狀態機）、AI 影像入庫 |
| 紀錄查詢 | 出庫/捐贈/報廢/轉移/AI 辨識紀錄、領取分析、Excel 匯出 |
| 系統管理 | 帳號管理、據點管理、庫存種類設定、LINE 通知設定、AI 智慧入庫設定 |
| 手機/LINE | 全螢幕手機頁面（物資查詢/領用/轉讓/影像入庫），供 LINE 圖文選單串接 |

角色：**管理員（Admin）／幹部（Cadre）／社工（SocialWorker）**，非管理員以「所屬據點」限縮資料範圍（RLS）。
