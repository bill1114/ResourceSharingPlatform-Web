...
export function AccountManagement() {
  ...
  async function load() { const [p, l] = await Promise.all([supabase.from('profiles').select('*').order('username'), supabase.from('supply_location').select('*').eq('is_active', true).order('id')]); setProfiles((p.data ?? []) as Profile[]); setLocations((l.data ?? []) as SupplyLocation[]) }
  useEffect(() => { void load() }, [])
  ...
  return <div className="container-fluid mt-4"><h2><i className="bi bi-people" /> 帳號管理</h2><hr />
    ...
    <div className="card shadow-sm"><div className="card-body"><input className="form-control mb-3" placeholder="搜尋帳號或顯示名稱" value={keyword} onChange={(e)=>setKeyword(e.target.value)}/><div className="table-responsive"><table className="table table-hover"><thead><tr><th>帳號</th><th>名稱</th><th>角色</th><th>據點</th><th>狀態</th><th /></tr></thead><tbody>{filtered.map(x=><tr key={x.id}><td>{x.username}</td><td>{x.display_name}</td><td>{roleDisplayName(x.role_name)}</td><td>{locations.find(l=>l.id===x.location_id)?.location_name ?? '未指定'}</td><td><span className={`badge bg-${x.is_active?'success':'secondary'}`}>{x.is_active?'啟用':'停用'}</span></td><td><button className="btn btn-outline-primary btn-sm" onClick={()=>edit(x)}>編輯</button></td></tr>)}</tbody></table></div></div></div>
  </div>
}
